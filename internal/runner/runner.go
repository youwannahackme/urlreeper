...
import (
	...
	"github.com/youwannahackme/urlreeper/pkg/engine"
	"github.com/youwannahackme/urlreeper/pkg/engine/headless"
	"github.com/youwannahackme/urlreeper/pkg/engine/hybrid"
	"github.com/youwannahackme/urlreeper/pkg/engine/standard"
...
func New(options *types.Options) (*Runner, error) {
	...
	if !options.DisableUpdateCheck {
		latestVersion, err := updateutils.GetToolVersionCallback("urlreeper", version)()
		if err != nil {
			if options.Verbose {
				gologger.Error().Msgf("urlreeper version check failed: %v", err.Error())
			}
		} else {
			gologger.Info().Msgf("Current urlreeper version %v %v", version, updateutils.GetVersionDescription(version, latestVersion))
		}
	}
