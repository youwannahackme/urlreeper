...
import (
	...
	"github.com/youwannahackme/urlreeper/pkg/engine/passive"
...
func (r *Runner) ExecuteCrawling() error {
	...
	if r.options.Passive {
		var passiveInputs []string
		ctx := r.options.Context
		if ctx == nil {
			ctx = context.Background()
		}
		for _, input := range inputs {
			passiveURLs, err := passive.Fetch(ctx, input)
			if err != nil {
				gologger.Warning().Msgf("Could not fetch passive URLs for %s: %s", input, err)
			}
			parsedInput, err := urlutil.Parse(input)
			var rootHostname string
			if err == nil {
				rootHostname = parsedInput.Host
			} else {
				rootHostname = input
			}
			for _, pURL := range passiveURLs {
				if inScope, _ := r.crawlerOptions.ValidateScope(pURL, rootHostname); inScope {
					passiveInputs = append(passiveInputs, pURL)
				}
			}
		}
		inputs = append(inputs, passiveInputs...)

		// Deduplicate inputs
		uniqueInputs := make(map[string]struct{})
		var dedupedInputs []string
		for _, input := range inputs {
			if _, ok := uniqueInputs[input]; !ok {
				uniqueInputs[input] = struct{}{}
				dedupedInputs = append(dedupedInputs, input)
			}
		}
